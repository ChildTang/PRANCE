...
class AttentionSuper(nn.Module):
    ...
    def set_sample_config(
        ...
        if not self.change_qkv:
            ...
            # print(sample_q_embed_dim, sample_in_embed_dim)
            self.sample_scale = (
    ...
    def forward(
        ...
        self.set_sample_config(
            sample_q_embed_dim, sample_num_heads, sample_in_embed_dim
        )
        ...
        # For Token Merging：Apply proportional attention
        attn = attn + token_size.log()[:, None, None, :, 0]
        ...
        re_info = {}
        re_info["attn"] = attn
        re_info["q"] = torch.mean(q, dim=1)
        re_info["k"] = torch.mean(k, dim=1)
        re_info["v"] = torch.mean(v, dim=1)
